...
hazardSE <- function(data, contr, var){
  ...
  f.fit0 <- safe_bshazard(Surv(age, dead) ~ 1, data = subset(temp, group == contr))
  f.fit1 <- safe_bshazard(Surv(age, dead) ~ 1, data = subset(temp, group == var))
  ...
  times <- seq(from = min(min(b0$time, na.rm = TRUE),
                          min(b1$time, na.rm = TRUE)),
               to   = max(max(b0$time, na.rm = TRUE),
                          max(b1$time, na.rm = TRUE)),
               by = 0.5)

  # control se
  bi0 <- as.data.frame(approx(x = b0$time, y = b0$hazard, xout = times))
  bl0 <- as.data.frame(approx(x = b0$time, y = b0$lowCI,  xout = times)); names(bl0)[2] <- "low"
  bu0 <- as.data.frame(approx(x = b0$time, y = b0$uppCI,  xout = times)); names(bu0)[2] <- "up"
  ...
  bl1 <- as.data.frame(approx(x = b1$time, y = b1$lowCI,  xout = times)); names(bl1)[2] <- "low"
  bu1 <- as.data.frame(approx(x = b1$time, y = b1$uppCI,  xout = times)); names(bu1)[2] <- "up"
  ...
  # variance + HR
  bi0 <- bi0[, c("x", "y", "se0")]; names(bi0) <- c("time", "h0", "se0")
  bi1 <- bi1[, c("x", "y", "se1")]; names(bi1) <- c("time", "h1", "se1")
